    ...
    const statusChange=async(id)=>{
      ...
      if(response.data.status=="Block")
      {
        const response =await axios.patch(`http://localhost:3000/user/${id}`,{status:"Unblock"});
          toast.success("User Unblock Sucessfully");
          fetch();
      }else{
        const response =await axios.patch(`http://localhost:3000/user/${id}`,{status:"Block"});
          toast.success("User Block Sucessfully");
          fetch();
      }
    }
            ...
            <div className="col-lg-12 grid-margin stretch-card">
              <div className="card">
                <div className="card-body">
                  <h4 className="card-title">User Manage</h4>
                          ...
                          data.map((item,index,ent)=>{
                            return (
                              <tr>
                                ...
                                <td>
								<span className='btn-danger' onClick={()=>{statusChange(item.id)}} style={{padding:'5px 15px',borderRadius:'7px'}}>{item.status=="Unblock"?"Block":"Unblock"}</span></td>
                              </tr>
                            );
                          })
                        }
